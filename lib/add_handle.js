...
exports.foreachAdd = function (numObj1, numObj2) {
    var result = {};
    if (numObj1.max < numObj2.max) {
        for (var i in numObj2) {
            if (i !== 'max') {
                if (numObj2.max === Number(i)) {
                    var sum = undefined.add(numObj1[i] || '0', numObj2[i]);
                    if (sum < 0) {
                    ...
                    result[i] = undefined.add(numObj1[i] || '0', numObj2[i]);
                }
            } else {
        ...
        for (var _i in numObj1) {
            if (_i !== 'max') {
                if (numObj1.max === Number(_i)) {
                    var _sum = undefined.add(numObj1[_i], numObj2[_i] || '0');
                    if (_sum < 0) {
                    ...
                    result[_i] = undefined.add(numObj1[_i], numObj2[_i] || '0');
                }
            } else {
...
exports.addNum = function (a, b) {
    ...
    var before = undefined.foreachAdd(objNumA.before, objNumB.before);
    var beforeP = '';
    var carry = 0;
    ...
    if (beforeP.length > pointLastIndexOf) {
        ...
        var reusltBefore = beforeP.slice(0, pointIndex - 1);
